...
import type { UserRole, ShiftType, UserStatus, EditUserPayload } from "@/types/users"

interface EditUserModalProps {
  open: boolean
  form: EditUserPayload
  errors: Partial<Record<keyof EditUserPayload, string>>
  onClose: () => void
  onSave: () => void
  onFormChange: (patch: Partial<EditUserPayload>) => void
}
...
export default function EditUserModal({ open, form, errors, onClose, onSave, onFormChange }: EditUserModalProps) {
  if (!open) return null
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/5 bg-zinc-900 text-white shadow-2xl">
        ...
        <div className="flex items-center gap-3 border-b border-white/5 px-6 py-5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">Update staff member details</p>
          </div>
          <button
            onClick={onClose}
